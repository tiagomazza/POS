...
# 1. Upload do listagem.xls
uploaded_file = st.file_uploader("Carregar listagem.xls", type=["xls", "xlsx"])
if uploaded_file is None:
    st.info("Por favor, carregue o ficheiro listagem.xls.")
    st.stop()

# Ler listagem.xls
listagem = pd.read_excel(uploaded_file, header=None)
listagem = listagem.iloc[4:, :]  # remover as primeiras 4 linhas
...
# Fazer join com revenda (se existir a coluna revenda)
...
# Ler POS_ABA.xls e criar POS
pos_aba = pd.read_excel("data/POS_ABA.xls")  # se precisares de algo dela, ajusta aqui
